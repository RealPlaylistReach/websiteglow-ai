...
/* ---------- screenshot -> base64 (key stays server-side) ----------
   Default: WordPress mShots — free, no key, no signup.
   Optional upgrade: ScreenshotOne, used automatically if SCREENSHOTONE_ACCESS_KEY is set. */
async function shotScreenshotOne(url) {
  const key = process.env.SCREENSHOTONE_ACCESS_KEY;
  if (!key) return null;
  const q = new URLSearchParams({
    access_key: key, url, viewport_width: "1280", viewport_height: "900",
    full_page: "false", format: "jpg", image_quality: "78",
  ...
  } catch { return null; } finally { clear(); }
}

async function shotMshots(url) {
  // mShots renders asynchronously: the first hit can return a small "generating" placeholder, so retry.
  const endpoint = "https://s0.wp.com/mshots/v1/" + encodeURIComponent(url) + "?w=1280&h=900";
  for (let i = 0; i < 3; i++) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (buf.length > 8000) return `data:${ct};base64,` + buf.toString("base64"); // real screenshot
        if (i === 2 && buf.length) return `data:${ct};base64,` + buf.toString("base64"); // last try: take what we got
      }
    } catch { /* retry */ } finally { clear(); }
    await wait(2500);
  }
  return null;
}
...
async function screenshot(url) {
  const pro = await shotScreenshotOne(url); // used only if a key is set
  if (pro) return pro;
  return await shotMshots(url); // free default
}
...
export default async (req) => {
  ...
  return json(200, {
    ...
    brand,
    content,
    warning: img ? undefined : "Live screenshot unavailable for this site (it may block previews) — try the Paste-HTML option.",
  });
};
