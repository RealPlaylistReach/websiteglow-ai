...
2. **Set environment variables** (Site settings → Environment variables):
   - `GEMINI_API_KEY` — **required, and FREE.** Get one (no credit card) at https://aistudio.google.com → "Get API key". This powers the AI redesign.
   - Screenshots default to **WordPress mShots — free, no key, nothing to set.** (Optional: set `SCREENSHOTONE_ACCESS_KEY` only if you later want higher-quality screenshots from screenshotone.com.)
   - Optional: to use Claude instead of Gemini for the redesign, set `ANTHROPIC_API_KEY` and remove `GEMINI_API_KEY`. Optional `REDESIGN_MODEL` / `GEMINI_MODEL` override the model.
