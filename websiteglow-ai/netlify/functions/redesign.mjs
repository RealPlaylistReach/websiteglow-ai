...
// Returns: { afterHtml }   (a self-contained, responsive, brand-faithful modern page)
...
async function callClaude(system, prompt, maxTokens) {
  ...
  if (!key) throw new Error("ANTHROPIC_API_KEY is not set in this site's environment.");
  const c = new AbortController();
...
export default async (req) => {
  ...
  let out;
  try {
    out = await callClaude(SYSTEM, buildPrompt(brand, content), 8000);
  } catch (e) {
  ...
  if (!/<\/html>|<\/body>/i.test(out)) {
    if (out.length > 600 && /<body/i.test(out)) out += "</body></html>"; // repair a truncated tail
